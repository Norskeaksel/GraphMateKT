...
4 2 5.0"""
    ...
    const val GRAPH_INFO =
        "Visualize a graph defined by edges from one node to another node with an optional weight. Example: Alice Bob 2.0."
    const val GRID_INPUT_INFO =
        "Visualize a grid defined by a rectangle string where each character represents a node. Example: abc."
    const val INT_GRAPH_INPUT_INFO =
        "Visualize an IntGraph defined by the nr of integer nodes and nr of edges, followed by edges with an optional weight. Example: 2 1, followed by 1 0 1.0."
    ...
    const val TARGET_NODE_INFO =
        "If the target node is set when running Dijkstra of BFS, the path from the start to the target node will be visualized after the search is complete."
